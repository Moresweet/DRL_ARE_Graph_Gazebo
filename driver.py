...
def writeToTensorBoard(writer, tensorboardData, curr_episode):
    ...
    reward, value, policyLoss, qValueLoss, entropy, policyGradNorm, qValueGradNorm, log_alpha, alphaLoss, travel_dist, success_rate, explored_area, object_reward = tensorboardData
    ...
    writer.add_scalar(tag='Perf/Explored Area', scalar_value=explored_area, global_step=curr_episode)
    writer.add_scalar(tag='Perf/Success Rate', scalar_value=success_rate, global_step=curr_episode)
    writer.add_scalar(tag='Perf/Object Reward', scalar_value=object_reward, global_step=curr_episode)
...
def main():
    ...
    log_alpha = torch.FloatTensor([-2]).to(
        device)  # not trainable when loaded from checkpoint, manually tune it for now
    ...
    metric_name = ['travel_dist', 'success_rate', 'explored_area', 'object_reward']
    training_data = []
    perf_metrics = {}
        ...
        while True:
            ...
            if curr_episode % 1 == 0 and len(experience_buffer[0]) >= MINIMUM_BUFFER_SIZE:
                ...
                for j in range(8):
                    ...
                    # SAC
                    with torch.no_grad():
                        q_values1, _ = dp_q_net1(node_inputs_batch, edge_inputs_batch, current_inputs_batch,
                                                 node_padding_mask_batch, edge_padding_mask_batch, edge_mask_batch)
                        q_values2, _ = dp_q_net2(node_inputs_batch, edge_inputs_batch, current_inputs_batch,
                                                 node_padding_mask_batch, edge_padding_mask_batch, edge_mask_batch)
                        q_values = torch.min(q_values1, q_values2)

                    logp = dp_policy(node_inputs_batch, edge_inputs_batch, current_inputs_batch,
                                     node_padding_mask_batch, edge_padding_mask_batch, edge_mask_batch)
                    ...
                    with torch.no_grad():
                        next_logp = dp_policy(next_node_inputs_batch, next_edge_inputs_batch, next_current_inputs_batch,
                                              next_node_padding_mask_batch, next_edge_padding_mask_batch,
                                              next_edge_mask_batch)
                        next_q_values1, _ = dp_target_q_net1(next_node_inputs_batch, next_edge_inputs_batch,
                                                             next_current_inputs_batch, next_node_padding_mask_batch,
                                                             next_edge_padding_mask_batch, next_edge_mask_batch)
                        next_q_values2, _ = dp_target_q_net2(next_node_inputs_batch, next_edge_inputs_batch,
                                                             next_current_inputs_batch, next_node_padding_mask_batch,
                                                             next_edge_padding_mask_batch, next_edge_mask_batch)
            ...
            if curr_episode % 5 == 0:
                ...
                if curr_episode % 50 == 0:
                    path_checkpoint = "./" + model_path + "/checkpoint.pth" + "_" + str(curr_episode)
